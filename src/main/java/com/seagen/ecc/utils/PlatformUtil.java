...
public class PlatformUtil {
	...
	static {
		...
		embedded3DEnabled = AccessController
				.doPrivileged(new PrivilegedAction<Boolean>() {
					@Override
					public Boolean run() {
						return Boolean
								.getBoolean("com.sun.javafx.experimental.embedded.3d");
					}
				});
		embeddedType = AccessController
				.doPrivileged(new PrivilegedAction<String>() {
					@Override
					public String run() {
	...
	private static final boolean ANDROID = "android".equals(System
			.getProperty("javafx.platform"));
	...
	private static void loadPropertiesFromFile(final File file) {
		...
		if (!foundPlatform) {
			System.err
					.println("Warning: No settings found for javafx.platform='"
							+ javafxPlatform + "'");
		}
	}

	/**
	 * Returns the directory containing the JavaFX runtime, or null if the
	...
	private static File getRTDir() {
		try {
			...
			// filename
			int lastIndexOfSlash = Math.max(s.lastIndexOf('/'),
					s.lastIndexOf('\\'));
			return new File(
					new URL(s.substring(0, lastIndexOfSlash + 1)).getPath())
					.getParentFile();
	...
	private static void loadProperties() {
		...
		if (!(javafxPlatform != null || (arch != null && arch.equals("arm")) || (vmname != null && vmname
				.indexOf("Embedded") > 0))) {
			return;
		}
		AccessController.doPrivileged(new PrivilegedAction<Void>() {
			public Void run() {
				...
				// First look for javafx.platform.properties in the JavaFX
				// runtime
				...
				File javaHomeProperties = new File(javaHome, "lib"
				...
				String javafxRuntimePath = System
						.getProperty("javafx.runtime.path");
